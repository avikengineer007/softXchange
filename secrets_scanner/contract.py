...
def merge_package_results(
    secrets_result: PackageScanResult,
    static_result: PackageScanResult,
) -> PackageScanResult:
    """Merges a secrets-scan result and a static-analysis result into one combined PackageScanResult.

    Status Decision Logic (Fail-Closed):
      - 'error': if either scanner failed with an unhandled exception or unparseable manifest
      - 'failed': if either scanner detected high/critical severity + high confidence findings
      - 'passed': if both scanners passed (findings remain included for seller review)

    Aggregations:
      - findings: union of all findings from both scanners
    ...
    """
    if secrets_result.status == ContractStatus.ERROR.value or static_result.status == ContractStatus.ERROR.value:
        overall_status = ContractStatus.ERROR.value
    elif secrets_result.status == ContractStatus.FAILED.value or static_result.status == ContractStatus.FAILED.value:
        overall_status = ContractStatus.FAILED.value
    else:
        overall_status = ContractStatus.PASSED.value

    merged_findings = list(secrets_result.findings) + list(static_result.findings)
    merged_suppressed = list(secrets_result.suppressed_findings) + list(static_result.suppressed_findings)

    merged_counts: Dict[str, int] = {"critical": 0, "high": 0, "medium": 0, "low": 0}
    for res in (secrets_result, static_result):
    ...
    combined_metadata = ScanMetadata(
        duration_seconds=round(secrets_result.metadata.duration_seconds + static_result.metadata.duration_seconds, 4),
        files_scanned=secrets_result.metadata.files_scanned + static_result.metadata.files_scanned,
        files_skipped=secrets_result.metadata.files_skipped + static_result.metadata.files_skipped,
        scan_history_ran=secrets_result.metadata.scan_history_ran or static_result.metadata.scan_history_ran,
    )

    errs = [e for e in (secrets_result.error_message, static_result.error_message) if e]
