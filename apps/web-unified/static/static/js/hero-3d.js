...
let ambientAngleY = 0.32;
let targetRotY = 0, currentTiltY = 0, currentTiltX = 0;
...
export async function initHeroScene(viewport) {
  ...
  const tierResult = getDeviceTier();
  const tier = typeof tierResult === 'string' ? tierResult : (tierResult?.tier || 'B');
  ...
  try {
    ...
    console.warn('[softXchange 3D] WebGL renderer setup failed, falling back to Tier C:', err);
    cleanupRenderer();
    renderStaticFallback(viewport);
    return;
  }
  ...
  try {
    ...
    const placeholder = viewport.querySelector('#hero-fallback, .hero-3d-placeholder');
    if (placeholder) {
      placeholder.remove();
    }
    const bowtie = viewport.querySelector('.hero-bowtie-target');
    if (bowtie) {
      bowtie.remove();
    }
    ...
    console.warn('[softXchange 3D] glTF load failed, falling back to Tier C:', err?.message || err);
    cleanupRenderer();
    renderStaticFallback(viewport);
    return;
  }
    ...
    loader.load(
      ...
      (gltf) => {
        try {
          ...
          model.rotation.y = ambientAngleY;
...
function renderLoop(now) {
  ...
  // Continuous ambient rotation + subtle parallax tilt
  if (model) {
    ambientAngleY += ROTATION_SPEED;
    currentTiltY += (targetRotY - currentTiltY) * PARALLAX_SMOOTH;
    currentTiltX += (targetParallaxY * 1.5 - currentTiltX) * PARALLAX_SMOOTH;

    model.rotation.y = ambientAngleY + currentTiltY;
    model.rotation.x = 0.12 + currentTiltX;
  }
