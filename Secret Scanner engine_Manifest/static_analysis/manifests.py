...
def parse_package_lock_json(content: str, file_path: str) -> Tuple[List[DeclaredDependency], Optional[Tuple[str, int]]]:
    """Parses package-lock.json for resolved dependencies (npm v1, v2, and v3 formats).
    ...
    # npm v2 / v3 lockfiles specify dependencies in the 'packages' map
    packages_map = data.get("packages")
    if isinstance(packages_map, dict):
        for pkg_path, details in packages_map.items():
            ...
            v_str = str(details.get("version", "")).strip() if details.get("version") is not None else ""
            line_no = 1
            search_key = f'"{pkg_path}"'
            for idx, line in enumerate(lines, start=1):
                if search_key in line:
                    line_no = idx
                    break
        ...
        # npm v1 lockfiles
        for pkg_name, details in data["dependencies"].items():
            if not isinstance(pkg_name, str) or not isinstance(details, dict):
                continue
...
def inspect_manifest(
    ...
    if filename == "package.json":
        ...
        deps, err = parse_package_lock_json(content, file_path)
