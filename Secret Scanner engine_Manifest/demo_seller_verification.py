...
def main():
    ...
    try:
        ...
        # ---------------------------------------------------------------------
        # SCENARIO 5: Package with manifest declaring an OSV-vulnerable dependency
        # ---------------------------------------------------------------------
        print("\n" + "-" * 70)
        print("SCENARIO 5: Upload package with manifest declaring an OSV-vulnerable dependency")
        print("-" * 70)

        # Mock OSV client queries for deterministic test execution
        import cve_check.osv_client
        orig_osv_query = cve_check.osv_client.OSVClient.query_dependencies

        def mock_osv_query(self, deps):
            paired = []
            for dep in deps:
                if dep.name == "lodash":
                    paired.append((
        ...
        cve_check.osv_client.OSVClient.query_dependencies = mock_osv_query

        cve_zip = os.path.join(temp_root, "cve_vulnerable.zip")
        with zipfile.ZipFile(cve_zip, "w") as zf:
            zf.writestr("app/service.py", "# Clean business logic\ndef run():\n    return True\n")
            zf.writestr("package.json", '{\n  "name": "cve-vulnerable-tool",\n  "version": "1.0.0",\n  "dependencies": {\n    "lodash": "4.17.15"\n  }\n}\n')

        cve_payload = {
            "listing_id": "listing_cve_vulnerable",
            "version_id": "v1.0",
            "seller_id": "seller_demo",
            "intake_source": "file_upload",
            "archive_path": cve_zip,
        }

        req = urllib.request.Request(
            f"{app.server_url}/api/intake/upload",
            data=json.dumps(cve_payload).encode("utf-8"),
            headers={"Content-Type": "application/json"},
            method="POST",
        )
        with urllib.request.urlopen(req) as resp:
        ...
        print("[*] Polling seller status endpoint until CVE scan finishes...")
        res_cve = poll_status(app.server_url, "listing_cve_vulnerable", "v1.0")

        print("\n[RESULT] Final Seller Status:")
        print(f"    Status:           {res_cve['status']}")
        print(f"    Storage Location: {res_cve['storage_location']}")
        print(f"    Severity Counts:  {res_cve['severity_counts']}")
        print(f"    Findings Count:   {len(res_cve['findings'])}")

        cve_rule_ids = [f['rule_id'] for f in res_cve['findings']]
        for idx, f in enumerate(res_cve['findings'], 1):
            print(f"      [{idx}] {f['rule_id']} in {f['file_path']}:{f['line_number']}")
            print(f"          Description: {f.get('description', '')}")
            print(f"          Remediation: {f.get('remediation_hint', '')}")

        assert res_cve["status"] == "scan_failed", f"Expected scan_failed, got {res_cve['status']}"
        assert not res_cve["storage_location"].startswith("live/"), "SECURITY BREACH: Vulnerable CVE package reached live/!"
        assert any("CVE-2020-8203" in rid or "GHSA-p6mc-m468-83gw" in rid for rid in cve_rule_ids), "Expected CVE finding in results"
        assert res_cve["severity_counts"].get("high", 0) >= 1, "Expected high severity count >= 1"
        print("\n>>> CONFIRMED: CVE scanner detected vulnerability via OSV, blocking listing from promotion to 'live'.")

        print("\n" + "=" * 70)
        print("ALL 5 END-TO-END SCENARIOS VERIFIED SUCCESSFULLY!")
        print("=" * 70)
        ...
        if 'orig_osv_query' in locals() and orig_osv_query is not None:
            cve_check.osv_client.OSVClient.query_dependencies = orig_osv_query
